...
bool checkInsideOfBoard(pos p, int boardSize) {
    return p.x >= 0 && p.x <= boardSize
           && p.y >= 0 && p.y <= boardSize;
}

// checks if given position is haven't marked before and inside of the board
bool isAvailable(int x, int y, char board[MAX_SIZE][MAX_SIZE], int boardSize) {
    pos p;
    p.x = x;
    p.y = y;
    return checkInsideOfBoard(p, boardSize) && board[x][y] == '.';
}
...
void fillNeighbors(pos arr[6], int x, int y) {
    arr[0].x = x;
    arr[0].y = y - 1;
    arr[1].x = x;
    arr[1].y = y + 1;
